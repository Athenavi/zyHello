...
const WXWORK_FIELDS: ConfigField[] = [
  { key: "WxworkCorpId", label: "Corp ID", type: "text", section: "credentials" },
  { key: "WxworkAgentId", label: "Agent ID", type: "text", section: "credentials" },
  { key: "WxworkSecret", label: "Secret", type: "password", section: "credentials" },
  { key: "WxworkToken", label: "Token", type: "text", section: "credentials", tip: "接收消息的 Token" },
  { key: "WxworkEncodingAESKey", label: "EncodingAESKey", type: "password", section: "credentials", tip: "接收消息的 EncodingAESKey" },
  { key: "WxworkSyncUsers", label: "自动同步用户", type: "boolean", section: "sync" },
  { key: "WxworkSyncUsersMatch", label: "用户匹配方式", type: "select", section: "sync", tip: "通过何种方式匹配已存在用户",
    options: [
      { value: "1", label: "用户名匹配" },
      { value: "2", label: "邮箱匹配" },
      { value: "3", label: "手机匹配" },
  ...
  { key: "WxworkSyncUsersRole", label: "新用户默认角色", type: "text", section: "sync" },
];
...
export default function WxworkConfigPage() {
  ...
  const loadConfig = async () => {
    setLoading(true);
    try {
      const data = await api.getIntegrationConfig("wxwork");
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.saveIntegrationConfig("wxwork", editValues);
  ...
  const sections = [...new Set(WXWORK_FIELDS.map((f) => f.section))];

  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">企业微信集成</h1>
          <p className="text-sm text-gray-500 mt-1">配置企业微信应用凭证和数据同步</p>
        </div>
            ...
            <div className="p-5">
              <table className="w-full">
                <tbody className="divide-y divide-gray-100">
                  {WXWORK_FIELDS.filter((f) => f.section === section).map((field) => (
                    <tr key={field.key}>
