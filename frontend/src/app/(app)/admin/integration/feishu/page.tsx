...
const FEISHU_FIELDS: ConfigField[] = [
  { key: "FeishuAppId", label: "App ID", type: "text", section: "credentials" },
  { key: "FeishuAppSecret", label: "App Secret", type: "password", section: "credentials" },
  { key: "FeishuEncryptKey", label: "Encrypt Key", type: "password", section: "credentials", tip: "事件订阅加密密钥" },
  { key: "FeishuVerificationToken", label: "Verification Token", type: "text", section: "credentials", tip: "事件订阅验证令牌" },
  { key: "FeishuSyncUsers", label: "自动同步用户", type: "boolean", section: "sync" },
  { key: "FeishuSyncUsersMatch", label: "用户匹配方式", type: "select", section: "sync", tip: "通过何种方式匹配已存在用户",
    options: [
      { value: "1", label: "用户名匹配" },
      { value: "2", label: "邮箱匹配" },
      { value: "3", label: "手机匹配" },
  ...
  { key: "FeishuSyncUsersRole", label: "新用户默认角色", type: "text", section: "sync" },
];
...
export default function FeishuConfigPage() {
  ...
  const loadConfig = async () => {
    setLoading(true);
    try {
      const data = await api.getIntegrationConfig("feishu");
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.saveIntegrationConfig("feishu", editValues);
  ...
  const sections = [...new Set(FEISHU_FIELDS.map((f) => f.section))];

  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">飞书集成</h1>
          <p className="text-sm text-gray-500 mt-1">配置飞书应用凭证和数据同步</p>
        </div>
            ...
            <div className="p-5">
              <table className="w-full">
                <tbody className="divide-y divide-gray-100">
                  {FEISHU_FIELDS.filter((f) => f.section === section).map((field) => (
                    <tr key={field.key}>
